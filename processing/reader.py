...
# python reader.py --inputDir /gpfs/projects/cms/fernanpe/ZprimeToMuMu_M-5000_TuneCP5_13TeV-madgraphMLM-pythia8/ZprimeToMuMu_M-5000_ntupler/200518_233322/0000/ 
...
if __name__ == '__main__':
    ...
    variables_muons = ['Muon_Eventid', 'Muon_Muonid', 'Muon_nHits', 'Muon_nGeomDets', 'Muon_Genpt', 'Muon_InnerTrack_pt', 'Muon_InnerTrack_eta', 'Muon_InnerTrack_phi', 'Muon_InnerTrack_charge', 'Muon_InnerTrack_ptErr', 'Muon_InnerTrack_Chindf', 'Muon_TunePTrack_pt', 'Muon_TunePTrack_ptErr']

    variables_hits = ['Hit_Eventid', 'Hit_Muonid', 'Hit_Hitid', 'Hit_Detid', 'Hit_isDT', 'Hit_isCSC', 'Hit_DTstation', 'Hit_CSCstation', 'Hit_DetElement', 'Hit_x', 'Hit_y', 'Hit_z', 'Hit_distToProp', 'Hit_Compatibility', 'Hit_dirx', 'Hit_diry', 'Hit_dirz', 'Hit_chi2', 'Hit_ndof']

    variables_props = ['Prop_Eventid', 'Prop_Muonid', 'Prop_Detid', 'Prop_isDT', 'Prop_isCSC', 'Prop_DTstation', 'Prop_CSCstation', 'Prop_DetElement', 'Prop_x', 'Prop_y', 'Prop_z']
    ...
    for File in os.listdir(args.inputDir):
        ...
        for event in tree:
            ...
            for entry in range(0,eval('event.' + variables_hits[0] + '.size()')):
                ...
                #print str(entry)
                line = str(eval('event.' + variables_hits[0] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[1] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[2] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[3] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[4] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[5] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[6] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[7] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[8] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[9] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[10] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[11] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[12] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[13] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[14] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[15] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[16] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[17] + '.at(' + str(entry) + ')'))  + '\t' + str(eval('event.' + variables_hits[18] + '.at(' + str(entry) + ')'))
