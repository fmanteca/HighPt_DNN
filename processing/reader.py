...
# python reader.py --inputDir /gpfs/projects/cms/fernanpe/ZprimeToMuMu_M-5000_TuneCP5_13TeV-madgraphMLM-pythia8/ZprimeToMuMu_M-5000_ntupler/200507_181639/0000/
...
if __name__ == '__main__':
    ...
    variables_hits = ['Hit_Eventid', 'Hit_Muonid', 'Hit_Hitid', 'Hit_Detid', 'Hit_isDT', 'Hit_isCSC', 'Hit_DTstation', 'Hit_CSCstation', 'Hit_DetElement', 'Hit_x', 'Hit_y', 'Hit_z', 'Hit_distToProp']
    ...
    for File in os.listdir(args.inputDir)[low:high]:
        ...
        for event in tree:
            ...
            for entry in range(0,eval('event.' + variables_hits[0] + '.size()')):
                ...
                #print str(entry)
                line = str(eval('event.' + variables_hits[0] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[1] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[2] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[3] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[4] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[5] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[6] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[7] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[8] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[9] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[10] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[11] + '.at(' + str(entry) + ')')) + '\t' + str(eval('event.' + variables_hits[12] + '.at(' + str(entry) + ')'))
